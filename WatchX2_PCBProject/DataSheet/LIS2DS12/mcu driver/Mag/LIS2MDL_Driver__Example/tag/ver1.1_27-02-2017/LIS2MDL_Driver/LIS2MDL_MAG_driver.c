...
* File Name          : LIS2MDL_MAG_driver.c
...
{
  //[Example]    
  HAL_I2C_Mem_Write(&hi2c1, LIS2MDL_MAG_I2C_ADDRESS, Reg, I2C_MEMADD_SIZE_8BIT, 
                    Bufp, len, 1000);      
  return MEMS_SUCCESS; 
}
...
{
  //[Example]
  HAL_I2C_Mem_Read(&hi2c1, LIS2MDL_MAG_I2C_ADDRESS, Reg, I2C_MEMADD_SIZE_8BIT, 
               Bufp, len, 1000);
  return MEMS_SUCCESS;                                  
}
...
status_t LIS2MDL_MAG_Set_InterruptThreshold(void *handle, u8_t *buff) 
{
  ...
  for (i=0; i<2;i++ ) 
  {
  if( !LIS2MDL_MAG_WriteReg(handle, LIS2MDL_MAG_INT_THS_L_REG+i,  &buff[i], 1) )
    return MEMS_ERROR;
  }
  return MEMS_SUCCESS;  
}
...
status_t LIS2MDL_MAG_Get_InterruptThreshold(void *handle, u8_t *buff) 
{
  ...
  {  
    if( !LIS2MDL_MAG_ReadReg(handle, LIS2MDL_MAG_INT_THS_L_REG+k, &buff[k], 1))
      return MEMS_ERROR;
    k++;  
  }
  }
