...
* File Name          : example_Sensor_Hub_LIS3MDL.c
...
LSM6DSL_ACC_GYRO_GDA_t value_G;
int AngularRate_mdps[MAX_PATTERN_NUM][3];

LSM6DSL_ACC_GYRO_XLDA_t value_XL;
...
static u16 Calculate_FIFO_Pattern(u16_t *min_odr, u16_t *max_odr)
{
  ...
  *min_odr = 0xffff;
  ...
  if (test_6dsl_gyro.t_enable) {
    LSM6DSL_ACC_GYRO_translate_ODR_G(test_6dsl_gyro.t_odr, &test_6dsl_gyro.t_odr_hz_val);
    *max_odr = MAX(*max_odr, test_6dsl_gyro.t_odr_hz_val);
    *min_odr = MIN(*min_odr, test_6dsl_gyro.t_odr_hz_val);
  }

  if (test_6dsl_xl.t_enable) {
    LSM6DSL_ACC_GYRO_translate_ODR_XL(test_6dsl_xl.t_odr, &test_6dsl_xl.t_odr_hz_val);
    *max_odr = MAX(*max_odr, test_6dsl_xl.t_odr_hz_val);
    *min_odr = MIN(*min_odr, test_6dsl_xl.t_odr_hz_val);
  }

  /* calculate how many samples for each sensor are in current FIFO pattern */

  if (test_6dsl_gyro.t_enable) {
    test_6dsl_gyro.t_samples_num_in_pattern = test_6dsl_gyro.t_odr_hz_val / *min_odr;
    test_6dsl_gyro.t_decimation =  *max_odr / test_6dsl_gyro.t_odr_hz_val;
    fifo_samples_tot_num += test_6dsl_gyro.t_samples_num_in_pattern;
  }

  if (test_6dsl_xl.t_enable) {
    test_6dsl_xl.t_samples_num_in_pattern = test_6dsl_xl.t_odr_hz_val / *min_odr;
    test_6dsl_xl.t_decimation =  *max_odr / test_6dsl_xl.t_odr_hz_val;
    fifo_samples_tot_num += test_6dsl_xl.t_samples_num_in_pattern;
  }
...
static void Read_FIFO_Pattern(u8 pattern_idx)
{
  u8_t gy_i = 0;
  u8_t xl_i = 0;
  u8_t gy_num = test_6dsl_gyro.t_samples_num_in_pattern;
  u8 gy_idx = pattern_idx * gy_num;
  u8_t xl_num = test_6dsl_xl.t_samples_num_in_pattern;
  ...
  while(gy_num > 0 || xl_num > 0) {
    ...
    if (test_6dsl_gyro.t_enable && gy_num > 0) {
      LSM6DSL_ACC_Get_AngularRate(0, AngularRate_mdps[gy_idx + gy_i++], 1);
      gy_num--;
    }

    /* read XL samples */
    if (test_6dsl_xl.t_enable && xl_num > 0) {
      LSM6DSL_ACC_Get_Acceleration(0, Acceleration_mG[xl_idx + xl_i++], 1);
      xl_num--;
    }
  }
}
 ...
 */
void LSM6DSL_ACC_GYRO_sample_Callback_fifo(u8_t intID)
{
  LSM6DSL_ACC_GYRO_FIFO_FULL_t full_ev;
  u16_t num = 0;

  LSM6DSL_ACC_GYRO_R_FIFOFull(0, &full_ev);
  if (LSM6DSL_ACC_GYRO_FIFO_FULL_FIFO_FULL == full_ev) {

    u16_t num_pattern = 0;
    u8 i = 0;

    LSM6DSL_ACC_GYRO_R_FIFONumOfEntries(0, &num);
 ...
 */
void LSM6DSL_ACC_GYRO_sample_Callback(u8_t intID)
{
  LSM6DSL_ACC_GYRO_XLDA_t event_XL;
  LSM6DSL_ACC_GYRO_GDA_t event_G;

  /*
   * Read ACC output only if new ACC value is available
   */
  LSM6DSL_ACC_GYRO_R_XLDA(0,&event_XL);
  if (LSM6DSL_ACC_GYRO_XLDA_DATA_AVAIL==event_XL) {
      LSM6DSL_ACC_Get_Acceleration(0, Acceleration_mG[0], 0);
  }

  /*
   * Read ACC output only if new ACC value is available
   */
  LSM6DSL_ACC_GYRO_R_GDA(0,&event_G);
  if (LSM6DSL_ACC_GYRO_GDA_DATA_AVAIL==event_G) {
    LSM6DSL_ACC_Get_AngularRate(0, AngularRate_mdps[0], 0);
  }
}
#endif

/* Init the Gyroscope */
static void init_LSM6DSL_GYRO(void)
{
...
static void init_LSM6DSL_ACC(void)
{
  /* Set ACC ODR  */
...
static  void Loop_Test_Sample_Aquisition(void)
{
  ...
  RegisterInterrupt(LSM6DSL_ACC_GYRO_sample_Callback_fifo);
  ...
  LSM6DSL_ACC_GYRO_W_FIFO_Watermark(0, FIFO_THRESHOLD);

  /* Force FIFO to stop on programmed threshold. */
  LSM6DSL_ACC_GYRO_W_STOP_ON_FTH(0, LSM6DSL_ACC_GYRO_STOP_ON_FTH_ENABLED);

  /* Route FIFO FULL event on INT1 */
  LSM6DSL_ACC_GYRO_W_FULL_FLAG_on_INT1(0, LSM6DSL_ACC_GYRO_INT1_FULL_FLAG_ENABLED);

  /*
   * Set FIFO ODR to high frequency. The device will use the maximum ODR among
   * sensors.
   */
  LSM6DSL_ACC_GYRO_W_ODR_FIFO(0, LSM6DSL_ACC_GYRO_ODR_FIFO_800Hz);

  /* Set FIFO in continuos mode */
  LSM6DSL_ACC_GYRO_W_FIFO_MODE(0, LSM6DSL_ACC_GYRO_FIFO_MODE_DYN_STREAM_2);

  /* Set FIFO Decimation according to pattern composition */
  LSM6DSL_ACC_GYRO_W_DEC_FIFO_XL_val(0, test_6dsl_xl.t_decimation);
  LSM6DSL_ACC_GYRO_W_DEC_FIFO_G_val(0, test_6dsl_gyro.t_decimation);

#else
  /* Config XL interrupt (Gyro samples will be acquired @ XL ODR) */
  RegisterInterrupt(LSM6DSL_ACC_GYRO_sample_Callback);

  /* Route XL DRDY event on INT1 */
  LSM6DSL_ACC_GYRO_W_DRDY_XL_on_INT1(0, LSM6DSL_ACC_GYRO_INT1_DRDY_XL_ENABLED);
#endif
  ...
  while(1)
  {
     ...
     * Read ACC output only if new ACC value is available
     */
    response =  LSM6DSL_ACC_GYRO_R_XLDA(0, &value_XL);
    if(response==MEMS_ERROR) while(1); //manage here comunication error  
    
    if (LSM6DSL_ACC_GYRO_XLDA_DATA_AVAIL==value_XL)
    {
      LSM6DSL_ACC_Get_Acceleration(0, Acceleration_mG[0], 0);
    }
     ...
     */
    response =  LSM6DSL_ACC_GYRO_R_GDA(0, &value_G);
    if(response==MEMS_ERROR) while(1); //manage here comunication error  
    
    if (LSM6DSL_ACC_GYRO_GDA_DATA_AVAIL==value_G)
    {
      LSM6DSL_ACC_Get_AngularRate(0, AngularRate_mdps[0], 0);
    }
  }
...
/* Init the Pedometer */
static void init_LSM6DSL_Pedometer(void)
{
  /* Set ACC ODR  */
 ...
 * It must be registered to be called at interrupt time.
 */
void LSM6DSL_ACC_GYRO_Tilt_Callback(u8_t intID)
{
  LSM6DSL_ACC_GYRO_TILT_EV_STATUS_t TiltStatus;

  LSM6DSL_ACC_GYRO_R_TILT_EV_STATUS(0, &TiltStatus);
  if (TiltStatus == LSM6DSL_ACC_GYRO_TILT_EV_STATUS_DETECTED) {
    Number_Of_Tilt_events++;

    /* handle event */
    TiltStatus = LSM6DSL_ACC_GYRO_TILT_EV_STATUS_NOT_DETECTED;
  }
}

/* Init the Tilt */
static void init_LSM6DSL_Tilt(void)
{
  /* Set ACC ODR  */
  response = LSM6DSL_ACC_GYRO_W_ODR_XL(0, LSM6DSL_ACC_GYRO_ODR_XL_26Hz);
  if(response==MEMS_ERROR) while(1); //manage here comunication error
  ...
  /* Enable Tilt  */
  response = LSM6DSL_ACC_GYRO_W_TILT(0, LSM6DSL_ACC_GYRO_TILT_ENABLED);
  if(response==MEMS_ERROR) while(1); //manage here comunication error

  /* Tilt on INT1 */
  response = LSM6DSL_ACC_GYRO_W_TiltEvOnInt1(0, LSM6DSL_ACC_GYRO_INT1_TILT_ENABLED);
  if(response==MEMS_ERROR) while(1); //manage here comunication error
...
#ifdef TEST_WITH_INTERRUPT
  RegisterInterrupt(LSM6DSL_ACC_GYRO_Tilt_Callback);
 ...
 * It must be registered to be called at interrupt time.
 */
void LSM6DSL_ACC_GYRO_SigMotion_Callback(u8_t intID)
{
  LSM6DSL_ACC_GYRO_SIGN_MOT_EV_STATUS_t SigMotStatus;

  LSM6DSL_ACC_GYRO_R_SIGN_MOT_EV_STATUS(0, &SigMotStatus);
  if (SigMotStatus == LSM6DSL_ACC_GYRO_SIGN_MOT_EV_STATUS_DETECTED) {
    Number_Of_SigMot_events++;

    /* handle event */
    SigMotStatus = LSM6DSL_ACC_GYRO_SIGN_MOT_EV_STATUS_NOT_DETECTED;
  }
}

/*
 * This test has been performed attaching a LIS3MDL magnetometer
 * to the I2C master bus provided by LSM6DSL device.
 *
...
#define LIS3MDL_SLAVE_ADDR    0x3C //0x3D //0x39
...
static void init_LSM6DSL_PT_SensorHub(void)
{
  u8 who_am_i = 0x0, Data;

  LSM6DSL_ACC_GYRO_W_FUNC_EN(0, LSM6DSL_ACC_GYRO_FUNC_EN_ENABLED);
  
  /* Enable internal pull up on sensor hub interface */
  LSM6DSL_ACC_GYRO_W_PULL_UP_EN(0, LSM6DSL_ACC_GYRO_PULL_UP_EN_ENABLED);
  
  /* Enable Passthru mode */
  LSM6DSL_ACC_GYRO_W_PASS_THRU_MODE(0, LSM6DSL_ACC_GYRO_PASS_THRU_MODE_ENABLED);
  ...
  LSM6DSL_ACC_GYRO_W_PASS_THRU_MODE(0, LSM6DSL_ACC_GYRO_PASS_THRU_MODE_DISABLED);
  
  /* Enable ACC x/y/x axis (for triggering the operation) */
  LSM6DSL_ACC_GYRO_W_ODR_XL(0, LSM6DSL_ACC_GYRO_ODR_XL_208Hz);
}
